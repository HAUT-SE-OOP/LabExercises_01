...
int main()
{
   ...
   double a = 10.0;
   double b = 20.0;
   SimpleCalculator sc; //定义一个 sc 对象
   ...
   double  addition= sc.add(a,b); // 进行加法操作
   cout << "Adding a and b yields " << addition << "\n";

   double subtraction = sc.subtract( a, b ); //进行减法操作
   cout << "Subtracting b from a yields " << subtraction << "\n";

   double multiplication = sc.multiply( a, b ); //进行乘法操作
   ...
   double division=sc.divide(a,b); //进行除法操作
