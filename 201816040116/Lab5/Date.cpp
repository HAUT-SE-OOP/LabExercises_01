...
void Date::nextday()
{
    setDay(day+1);      //Add a day
    if(day==1)      //Judge whether the increase is the first to determine whether the month changes
        month=month+1;
    setMonth(month);
    if(month==1&&day==1)    //Judge whether the increase is January 1 to determine whether the year changes
        year=year+1;
    setYear(year);
}
bool Date::leapYear()
