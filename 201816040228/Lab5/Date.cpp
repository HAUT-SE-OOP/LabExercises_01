...
Date::~Date()
{
    cout << "Date class was called successfully" << endl;
}
...
void Date::nextDay()
{
    ...
    if(month == 12 && day+1>monthDays())//12月31日,跨年
    {
        day=0;
        month=1;
        year++;
    }
    if(month >=1 && month <= 11 && day+1>monthDays())//每个月的月末，不跨年
    {
        day=1;
        month++;
    }
...
//define member function nextDay
