...
{
    //setDay(day+1);
    if(day==monthDays())//if the day is the last day of this month
    {
        day=0;//first make the day be 0
        if(month==12)//if the month is the last month of a year
        {
            setYear(year+1);//the data of the year should add to 1
            setMonth(1);//the month is 1
        }
        else
            setMonth(month+1);//if the month is other months,the data add to 1

    }
    setDay(day+1);//the data of the day add to 1
}
