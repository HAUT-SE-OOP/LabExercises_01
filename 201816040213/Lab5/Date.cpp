...
void Date::nextday()
{
    this->day++; //days plus one
    if(day>monthDays()) //If the day bigger than monthDays,day reset to one,month plus one
    {
        this->day=1;
        this->month+=1;
    }
    if(month>12) //If the month is bigger than 12,month reset to one,year plus one
    {
        this->month=1;
        this->year+=1;
    }
