...
void Date::nextday()
{
    day = getDay() + 1 ;//day add one
    if(getDay() > monthDays())//if the day is greater than monthDays,The number of days is reset to 1,month add one
    {
        day=1;
        month = getMonth() +1;
     if(getMonth() >12 )//if the month is greater than 12,The number of month is reset to 1,year add one
    {
        year = getYear()+1;
        month = 1;
    }
